...
class NewsPagingSource(
    private val api: Api
) : PagingSource<Int, NewsSerializable>() {
    private final val ITEM_AMOUNT_PER_SKIP = 5

    override fun getRefreshKey(state: PagingState<Int, NewsSerializable>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, NewsSerializable> {
        return try {
            val skip = params.key ?: 0
            val response = api.getNews(skip = skip)
